<template>
<v-app>
    <!-- <v-card class="overflow-hidden p-0 mb-0" height="100%" min-height="100%"> -->
    <v-app-bar app clipped-left clipped-right dark dense flat shrink-on-scroll :src="require('./assets/omma.jpg').default" fade-img-on-scroll scroll-target="#scrolling-techniques-3">
        <v-app-bar-nav-icon @click.stop="drawer = !drawer"></v-app-bar-nav-icon>

        <router-link class="white--text title" to="/">
            {{_settings('general').websit_title}}
...
<v-icon>mdi-menu</v-icon>
        ...
        <template v-slot:extension>
            <v-tabs align-with-title>
                <v-tab style="width: 11rem" to="/login2">تسجيل الدخول
                ...
                <div v-for="(item, index) in routes" :key="index">
                    <template v-if="item.children">
                        <v-list-group v-if="
                                        ...
                                        " :group="item.path" no-action :key="item.name" :prepend-icon="item.icon" active-class="cls">
                            ...
                            <template v-for="link in item.children">
                                <v-link v-if="
                                                ...
                                                " :key="link.id" :tolink="link.path" :title="
                                                    $t(
                                                        link.label
                                                            ? link.label
                                                            : link.name
...
<v-progress-circular v-if="$store.state.loading" color="white" indeterminate></v-progress-circular>
    ...
    <!-- <v-speed-dial v-model="fab" absolute top left direction="top" transition="slide-y-reverse-transition" style="top: 50%; left: 0%   ">
...
<v-icon>mdi-twitter</v-icon>
    ...
    <!-- <v-slider
                ...
                thumb-label
            ></v-slider> -->
    <!-- <v-speed-dial absolute bottom right direction="top" transition="slide-y-reverse-transition">

<template v-slot:activator>

<v-btn v-model="fab" color="blue darken-3" dark fab tile @click="$vuetify.goTo(-999, options)">
...
mdi-arrow-up-bold-circle
    ...
    <v-sheet id="scrolling-techniques-3" class="overflow-y-auto p-0 yourCard" max-height="100vh" min-height="100vh" style="overflow-x: hidden !important">
